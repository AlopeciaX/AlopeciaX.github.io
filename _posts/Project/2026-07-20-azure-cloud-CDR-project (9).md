...
<sub>[`tuna-autoblock-playbook` 워크플로우 — 트리거·HTTP 액션 연결 완료 상태]</sub>

![](../../assets/images/Project/2026-07-20-azure-cloud-CDR-project/file-20260714113149266.png)
<sub>[HTTP 액션 PATCH 요청 설정 — URI/Method/Body]</sub>
...
<sub>[PATCH 실패 — `OnlyTagsSupportedForPatch` 오류]</sub>
...
<sub>[PUT 전환 후 `LocationRequired` / 규칙셋 소실 오류 확인]</sub>

공격자 IP는 Sentinel Incident의 `relatedEntities[0].properties.address` 경로에 담겨 있는 것을 실제 트리거 payload로 확인해 동적 콘텐츠로 연결했다.

SSRF를 다시 재현해 실제로 WAF 커스텀 룰에 공격자 IP가 자동 추가되는 것을 확인했다. 이후 같은 공인 IP로는 사이트 접속이 차단되는 것까지 직접 검증했다 (`http://` 접속은 차단되고 `https://`는 애초에 443 포트 자체가 없어 별개로 연결 불가였던 것도 함께 확인).

![](../../assets/images/Project/2026-07-20-azure-cloud-CDR-project/file-20260714121237023.png)
<sub>[수정된 완전한 Body로 PUT 성공 — WAF 정책에 `AutoBlock` 룰 자동 추가 확인]</sub>
...
<sub>[실제 필드값 확인 — `event_class_s: connection_log`, `event_subclass_s: CONNECT`]</sub>
...
<sub>[수정된 쿼리로 재조회 — `fakeuser` 접속 시도 6건 정확히 매치]</sub>

![](../../assets/images/Project/2026-07-20-azure-cloud-CDR-project/file-20260714143818176.png)
<sub>[규칙 배포 상태 확인 — `enabled: true`, 수정된 쿼리 반영 확인]</sub>
...
<sub>[`az rest`로 조회한 Incident 생성 확인 — `created` 타임스탬프 포함]</sub>
