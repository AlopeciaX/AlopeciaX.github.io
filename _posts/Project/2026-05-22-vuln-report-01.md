...
**Windows** — 가장 치명적이었던 **EternalBlue SMB RCE(MS17-010)**부터 직접 익스플로잇해 곧바로 SYSTEM 권한을 획득했다. 이어서 ManageEngine DC FileUploadServlet RCE(CVE-2015-8249)와 BlueKeep RDP RCE(CVE-2019-0708), JMX 비인증 RCE로 원격 코드 실행에 성공했고, TrackPopupMenu 취약점으로 일반 권한에서 SYSTEM까지 권한 상승을 재현했다. 그 외 Jenkins Script Console, Rails RCE, Elasticsearch RCE, Tomcat PUT JSP, PHP CGI RCE, Axis2/SSH/FTP/GlassFish/MySQL 기본 자격증명 등 총 15개 경로로 직접 침투를 검증했다.

![](../../assets/images/Project/2026-05-22-vuln-report-01/file-20260703141143037.png)

**Linux** — **ProFTPD mod_copy(CVE-2015-3306)**로 인증 없이 웹쉘을 업로드하는 것부터 시작해, Drupal Coder RCE와 UnrealIRCd 백도어(CVE-2010-2075)로 원격 명령 실행까지 성공시켰다. 마지막으로 **PwnKit(CVE-2021-4034)**을 활용해 일반 권한에서 root까지 권한을 상승시켰다. 그 외 Drupal SQLi, HTTP PUT/DELETE, SSH/FTP 기본 자격증명, Apache Continuum RCE 등 총 10개 경로로 침투를 검증했다.
...
진단 결과를 바탕으로 취약점별 대응 방안을 직접 정리했다. SMB EternalBlue는 MS17-010 패치 적용과 SMBv1 비활성화, RDP BlueKeep은 최신 패치 적용과 외부 노출 차단을 권고했다. ProFTPD mod_copy는 1.3.5a 이상 업그레이드 또는 mod_copy 비활성화를, Drupal Coder RCE는 불필요한 개발용 모듈 삭제를 제안했다. UnrealIRCd 백도어는 정상 버전으로 즉시 교체, 곳곳에서 발견된 기본 자격증명 문제는 비밀번호 변경과 SSH 키 기반 인증 전환을, PwnKit은 PolicyKit 0.105-33 이상 업그레이드와 pkexec SUID 비트 제거를 권고했다.
