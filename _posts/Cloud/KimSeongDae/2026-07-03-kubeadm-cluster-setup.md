...
**환경**: Rocky Linux 9, VMware Workstation (Full Clone) **노드 구성**
...
**구성 요소**: kubeadm, containerd, Calico(CNI), MetalLB(LoadBalancer) **Kubernetes 버전**: v1.35.6
...
Rocky Linux 9 기본 저장소에는 `containerd` 패키지가 없어 Docker CE 저장소가 필요함 (`No match for argument: containerd` 에러 발생했던 부분)
...
**트러블슈팅**: worker1에서 calico-node 파드가 `Init:0/3`에서 수 분간 멈췄던 적 있음 (이미지 pull 지연). 결국 자동으로 해소되어 Running으로 전환됨. 원인 미상 시 확인 명령:
...
2. **DB 연결 실패 (`Error establishing a database connection`)**:
    ...
    - 확인 명령:
        ...
        ```bash
        kubectl logs -l tem=mysql --tail=50kubectl exec -it <wordpress-pod> -- bashmysql -h mysql-service -u jhjang -p word
        ```
        
    - `ERROR 2005 (HY000): Unknown server host 'mysql-service'` 발생 시 → DNS(CoreDNS) 또는 Service/Endpoints 문제
        
        ```bash
        kubectl get endpoints mysql-service   # 비어있으면 selector 불일치kubectl get pods -n kube-system -l k8s-app=kube-dnsnslookup kubernetes.default   # 파드 내부에서 DNS 자체 동작 확인
        ```
