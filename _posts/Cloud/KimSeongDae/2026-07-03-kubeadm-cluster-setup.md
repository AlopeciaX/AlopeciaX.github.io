...
Deployment 라벨은 `tem=word`, Service selector가 `tem=wordpress`로 달라서 트래픽 라우팅 안 됨 → selector를 `tem: word`로 일치시켜 해결
...
`ERROR 2005 (HY000): Unknown server host 'mysql-service'` 발생 시 → DNS(CoreDNS) 또는 Service/Endpoints 문제 확인:
...
|`kubectl apply --dry-run=server` 후 리소스가 안 보임|dry-run은 실제 생성 안 함(검증만)|dry-run 옵|
