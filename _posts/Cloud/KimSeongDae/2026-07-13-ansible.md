---
title: ansible
date: 2026-07-13
categories:
  - cloud
comments: true
tags:
  - ansible
---
---
ad-hoc

ansible -i /etc/ansible/hosts

ansible all -m ping
(ansible 모듈)

---
hostnamectl set-hostname cont

hostnamectl set-hostname node1
hostnamectl set-hostname node2
hostnamectl set-hostname node3

ssh-keygen -m PEM -t rsa -b 2048 -q -N ""

![](../../../assets/images/Cloud/KimSeongDae/2026-07-13-ansible/file-20260713093845065.png)

scp .ssh/id_rsa.pub root@10.0.0.12:/root/.ssh/authorized_keys
scp .ssh/id_rsa.pub root@10.0.0.13:/root/.ssh/authorized_keys
scp .ssh/id_rsa.pub root@10.0.0.14:/root/.ssh/authorized_keys
scp .ssh/id_rsa.pub root@10.0.0.11:/root/.ssh/authorized_keys

![](../../../assets/images/Cloud/KimSeongDae/2026-07-13-ansible/file-20260713094113024.png)

dnf install -y epel-release
dnf install -y ansible
ansible -version

![](../../../assets/images/Cloud/KimSeongDae/2026-07-13-ansible/file-20260713094516013.png)

- ansible 설치 후 인벤토리 생성
vi /etc/ansible/hosts

```
[all]
10.0.0.[11:14]

[node]
10.0.0.12
10.0.0.13
10.0.0.14

[web]
10.0.0.12

[was]
10.0.0.13

[db]
10.0.0.14

[http:children]
web
was
```

ansible -i /etc/ansible/hosts all -m ping
ansible all -m ping 으로 해도 됨

![](../../../assets/images/Cloud/KimSeongDae/2026-07-13-ansible/file-20260713094826216.png)

ansible http -m ping

![](../../../assets/images/Cloud/KimSeongDae/2026-07-13-ansible/file-20260713094911636.png)
ansible playbook


ansible 최상위에 test 디렉토리 생성
역등성보장 스크립트파일은 역등성보장x

terraform provisioner

ansible node -m file -a "path=/test state=directory"
ansible node -m file -a "path=/test state=absent"


ansible node -m shell -a "ls -al /'

ansible node -m shell -a "useradd a"
ansible node -m shell -a "tail -5 /etc/passwd"

ansible node -m shell -a "userdel -r a"

ansible node -m shell -a "tail -5 /etc/passwd"
ansible node -m shell -a "ls -al /home"
ansible node -m shell -a "ls -al /var/spool/mail"

ansible node -m shell -a "useradd a; echo 'It1' | passwd --stdin a"
ansible node -m shell -a "tail -5 /etc/passwd"
ansible node -m user -a "name=b"
ansible node -m shell -a "tail -5 /etc/passwd"
ansible node -m user -a "name=n update_password=always password={{ 'It1' | password_hash('sjha512') }}"
ansible node -m shell -a "tail -5 /etc/shadow"

mkdir /file
cd /file
vi test.yml
ansible-playbook test.yml

alias ap='ansible playbook'
ap test.yml

- mame: make file /test
	hosts: node
	gather_facs: false
	ingore_errors: ture

tasks:
-name: makke file test

ifle: 
	pasth:/etst
	state:touch
	mode: '0777'

-name make directory /tbabo
file: 
 path:/tbabo
 directroy
 0777
ap test.yml
ansible node -m shell -a "ls -al /"

ansible node -a skell

test 권한 777 설정

cd file

ansible vi user.yml

-name: create user a & password 'It1'
hosts: web
gatther_facs: false
ignore_errors: true

tasaks:
-name: create user a
user:
	name: 

ap user.yml

ansible web -m shell -a "tai -5 /tec/shadow"

