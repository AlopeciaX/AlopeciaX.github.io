...
Metadata cleanup
Connections
Connect to server w2k22-ad.jhjang.local
...
Select server 0
List Naming Contexts # 높은번호 선택
Select naming context 6

```

	삭제할때는 서버 -> 명명컨텍스트 -> 도메인 순으로 꼭 지워야 함
...
mem3가서 자식 busan 재생성 시도

그리고 삭제할땐 DNS 주영역을 상대방의 보조영역으로 또 설정하고 삭제해야 깔끔하게 삭제됨
...
a라는 계정이 w10에서만 관리자 계정을 가지게 하려면 어떻게 해야할까?
...
로컬 -> Site -> Domain -> OU(Organization Unit - 가장 작은 단위)
...
`gpupdate /force` : 강제로 업데이트 -> 로그아웃 -> 다시 접속
...
부모 DC에서 자식 DC로 전송할 수 있는 FSMO는 몇개일까요?

	도메인 네임 마스터, 스키마 마스터, (P, RID, ID)
